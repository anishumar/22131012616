...
# Log app startup
Logger.info(stack="main.py", package="FastAPI", message="App startup.")
...
class DatabaseHandler:
    ...
    def save(self, item: Item) -> str:
        ...
        if item.shortcode:
            if not self._is_valid_shortcode(item.shortcode):
                Logger.warning(stack="main.py", package="DatabaseHandler", message=f"Invalid shortcode format: {item.shortcode}")
                raise ValueError("Shortcode must be alphanumeric and 4-16 characters long.")
            if item.shortcode in self.data:
                Logger.warning(stack="main.py", package="DatabaseHandler", message=f"Shortcode {item.shortcode} already exists.")
            ...
            while True:
                ...
                attempts += 1
                if attempts > 10_000:
                    Logger.fatal(stack="main.py", package="DatabaseHandler", message="Failed to generate unique shortcode after 10,000 attempts.")
        ...
        Logger.info(stack="main.py", package="DatabaseHandler", message=f"Shortcode {shortcode} saved.")
        return shortcode

    def get(self, shortcode: str):
        item = self.data.get(shortcode)
        if not item:
            Logger.warning(stack="main.py", package="DatabaseHandler", message=f"Shortcode {shortcode} not found.")
            return None
        if datetime.datetime.now(datetime.timezone.utc) > item["expiry"]:
            del self.data[shortcode]
            Logger.info(stack="main.py", package="DatabaseHandler", message=f"Shortcode {shortcode} expired and deleted.")
            return None
        Logger.debug(stack="main.py", package="DatabaseHandler", message=f"Shortcode {shortcode} retrieved.")
        return item

    def get_stats(self, shortcode: str):
        item = self.data.get(shortcode)
        if not item:
            Logger.warning(stack="main.py", package="DatabaseHandler", message=f"Stats requested for non-existent shortcode {shortcode}.")
            return None
        stats = {
    ...
    def record_click(self, shortcode: str, referrer: Optional[str], location: Optional[str]):
        ...
        if item:
            ...
            Logger.info(stack="main.py", package="DatabaseHandler", message=f"Click recorded for shortcode {shortcode}.")

_database_handler = DatabaseHandler()

@app.get("/")
async def root():
    Logger.info(stack="main.py", package="Endpoint", message="GET / called.")
    return {"status": "online"}

@app.post("/shorturls", response_model=ItemResponse, status_code=201)
async def create_short_url(item: Item, request: Request):
    Logger.info(stack="main.py", package="Endpoint", message="POST /shorturls called.")
    try:
        ...
        Logger.info(stack="main.py", package="Endpoint", message=f"Short URL created: {short_url}")
        return ItemResponse(shortLink=short_url, expiry=expiry)
    except ValueError as e:
        Logger.error(stack="main.py", package="Endpoint", message=f"Error creating short URL: {str(e)}")
        return JSONResponse(status_code=409, content={"error": str(e)})
    except Exception as e:
        Logger.fatal(stack="main.py", package="Endpoint", message=f"Unexpected error: {str(e)}")
...
async def get_short_url_stats(shortcode: str):
    Logger.info(stack="main.py", package="Endpoint", message=f"GET /shorturls/{shortcode} (stats) called.")
    stats = _database_handler.get_stats(shortcode)
    if not stats:
        Logger.warning(stack="main.py", package="Endpoint", message=f"Stats not found for shortcode {shortcode}.")
...
async def redirect_short_url(shortcode: str, request: Request, response: Response):
    Logger.info(stack="main.py", package="Endpoint", message=f"GET /{shortcode} (redirect) called.")
    item = _database_handler.get(shortcode)
    if not item:
        Logger.warning(stack="main.py", package="Endpoint", message=f"Shortcode {shortcode} not found for redirection.")
        return JSONResponse(status_code=404, content={"error": "Shortcode not found"})
    if datetime.datetime.now(datetime.timezone.utc) > item["expiry"]:
        Logger.warning(stack="main.py", package="Endpoint", message=f"Shortcode {shortcode} has expired.")
    ...
    Logger.info(stack="main.py", package="Endpoint", message=f"Redirecting shortcode {shortcode} to {item['url']}")
